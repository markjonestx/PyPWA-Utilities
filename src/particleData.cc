...
void
particleData::_init(string nm, double m, double w, int i, int g, int j, int p,
                    int c) {
    _name = nm;
    _mass = m;
    _width = w;
...
particleData::particleData(const particleData &p) {
    ...
    if (_particle_data_debug) {
        cerr << "in particleData(" << this
             << ")::particleData(const particleData& p=" << p.Name()
             << ")\t_particle_data_count = " << _particle_data_count << endl;
    }
    _init(p._name, p._mass, p._width, p._isospin, p._gparity, p._spin,
          p._parity, p._cparity);
}

particleData::particleData(string n, double m, double w, int i, int g, int j,
                           int p, int c) {
    _particle_data_count++;
    if (_particle_data_debug) {
        cerr << "in particleData(" << this
             << ")::particleData(string ,double, double, int, int, int, int, int)\t_particle_data_count = "
             << _particle_data_count << endl;
    }
...
particleData &particleData::operator=(const particleData &p) {
    if (_particle_data_debug) {
        cerr << "in particleData(" << this
             << ")::operator=(const particleData&(" << &p << ")=" << p.Name()
             << ")\t_particle_data_count = " << _particle_data_count << endl;
    }
    _init(p._name, p._mass, p._width, p._isospin, p._gparity, p._spin,
          p._parity, p._cparity);
...
particleData::~particleData() {
    _particle_data_count--;
    if (_particle_data_debug) {
        cerr << "in particleData(" << this << ")=" << this->Name()
             << ")::~particleData()\t_particle_data_count = "
             << _particle_data_count << endl;
    }
}
...
void particleData::print() const {
    ...
    cout << "(" << this->_spin << getsign(this->_parity)
...
void particleDataTable::initialize(char *PDTfile) {
    ...
    while (!(ifs >> name).eof()) {
        ...
        ifs >> parity;
        ifs >> cparity;
        insert(particleData(name, mass, width, isospin, gparity, spin, parity,
                            cparity));
    }
