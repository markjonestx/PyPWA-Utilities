...
class wave: public particle {
  ...
  public:
    wave():particle() {this->_b = 0.0; this->_t = 0.0;}
    wave(const wave& wv);
    ~wave() {;}
    wave& operator=(const wave&);

    wave setM(int m) { this->_m = m; return(*this); }
    wave setSlope(double b) { this->_b = b; return(*this); }
    wave setT(double t) { this->_t = t; return(*this); }
    fourVec getBeam() const { return this->_beam; } 
    fourVec getTarget() const { return this->_target; } 
    int M() { return(this->_m); }
    wave channel(char* c) { this->_channel = c; return(*this); }
